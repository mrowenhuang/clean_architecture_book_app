...
Widget literatureBook() {
  return BlocBuilder<BookmarkBloc, BookmarkState>(
    builder: (context, bookmarkState) {
      return BlocBuilder<LiteratureBloc, LiteratureState>(
        bloc: context.read<LiteratureBloc>()..add(GetLiteratureBookEvent()),
        builder: (context, state) {
          if (state is LoadingGetLiteratureBookState) {
            return Center(
              child: CupertinoActivityIndicator(color: AppColor.primary),
            );
          } else if (state is SuccessGetLiteratureBookState) {
