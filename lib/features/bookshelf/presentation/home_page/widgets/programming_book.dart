...
Widget programmingBook() {
  return BlocBuilder<BookmarkBloc, BookmarkState>(
    builder: (context, bookmarkState) {
      return BlocBuilder<ProgrammingBloc, ProgrammingState>(
        bloc: context.read<ProgrammingBloc>()..add(GetProgrammingBookEvent()),
        builder: (context, state) {
          if (state is LoadingGetProgrammingBookState) {
            return Center(
              child: CupertinoActivityIndicator(color: AppColor.primary),
            );
          } else if (state is SuccessGetProgrammingBookState) {
