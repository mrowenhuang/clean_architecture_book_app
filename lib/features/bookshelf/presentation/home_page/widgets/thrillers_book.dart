...
Widget thrillersBook() {
  return BlocBuilder<BookmarkBloc, BookmarkState>(
    builder: (context, bookmarkState) {
      return BlocBuilder<ThrillersBloc, ThrillersState>(
        bloc: context.read<ThrillersBloc>()..add(GetThrillersBookEvent()),
        builder: (context, state) {
          if (state is LoadingGetThrillersBookState) {
            return Center(
              child: CupertinoActivityIndicator(color: AppColor.primary),
            );
          } else if (state is SuccessGetThrillersBookState) {
